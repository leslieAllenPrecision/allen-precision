...
class MagentoFinancialStatusEpt(models.Model):
    ...
    def search_financial_status(self, order_response, magento_instance, payment_option):
        ...
        status_name = self.__get_status_name(order_response.get('status'), is_invoice, is_shipment)
        status_code = self.__get_status_code(order_response.get('status'), is_invoice, is_shipment)
